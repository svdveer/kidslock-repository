...
def init_db():
    ...
    # Bugfix: Zorg dat kolommen altijd bestaan zonder data te wissen
    ...
    conn.commit()
    conn.close()
    logger.info("Database gecheckt en klaar.")
    ...
    try:
        ...
        for ip, name, elapsed in tvs:
            ...
            if topic == "kidslock/set" or topic == f"kidslock/{slug}/set":
                if payload.startswith("+"): # Bonus tijd via MQTT
                    mins = int(payload.replace("+", ""))
                    with sqlite3.connect(DB_PATH) as c:
                        c.execute("UPDATE tv_configs SET elapsed = max(0, elapsed - ?) WHERE ip = ?", (mins, ip))
                    logger.info(f"Bonus: {mins}m toegevoegd aan {name}")
                else:
...
async def tv_action(ip: str = Form(...), action: str = Form(...)):
    if action == "reset":
        ...
        mins = int(action.replace("+", ""))
        with sqlite3.connect(DB_PATH) as conn:
            conn.execute("UPDATE tv_configs SET elapsed = max(0, elapsed - ?) WHERE ip = ?", (mins, ip))
        return {"status": "ok"}
    try:
...
async def pair(ip: str=Form(...), code: str=Form(...)):
    try:
        # Pairing logic inclusief API sleutel generatie
    ...
    except Exception as e: logger.error(f"Pairing error: {e}")
