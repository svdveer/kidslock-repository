...
# --- MONITOR LOOP (Tijd bijhouden) ---
def monitor_task():
    logger.info("Monitor gestart...")
    last_tick = time.time()
    while True:
        try:
            now = datetime.now()
            today = now.strftime("%Y-%m-%d")
            delta = (time.time() - last_tick) / 60.0 # Tijd in minuten
            last_tick = time.time()
            ...
            for name, ip, no_limit, elapsed, last_reset in tvs:
                ...
                # Check of apparaat online is via poort 8081
                online = False
                try:
                ...
                # Als online en geen onbeperkte tijd: tel op
...
async def home(request: Request):
    ...
    tvs_data = []
    for r in rows:
        limit = 120 # Standaard 2 uur
        tvs_data.append({
            "name": r[0],
            "ip": r[1],
            "elapsed": round(r[2], 1),
            "no_limit": r[3],
            "remaining": int(max(0, limit - r[2])) # Voorkomt de Jinja2 error
        })
        ...
        if r.status_code == 200:
            with sqlite3.connect(DB_PATH) as conn:
                conn.execute("INSERT OR REPLACE INTO tv_configs (name, ip, no_limit, elapsed, last_reset) VALUES (?, ?, ?, 0, ?)", 
                             (f"Android_TV_{ip.split('.')[-1]}", ip, 0, datetime.now().strftime("%Y-%m-%d")))
