...
# Laden van opties met fallback naar lege lijst
...
for tv in options.get("tvs", []):
    # Gebruik standaard 120 min als daily_limit ontbreekt
...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        logger.error(f"❌ MQTT Fout code {rc}. Check credentials.")
    ...
    with data_lock:
        for tv_name, state in tv_states.items():
            ...
            if msg.topic == f"kidslock/{slug}/set":
                ...
                control_tv(tv_name, action, "MQTT Remote")
                state["manual_override"] = True

mqtt_client.on_connect = on_connect
mqtt_client.on_message = on_message

# Gebruik Supervisor Credentials
...
try:
    ...
    logger.error(f"MQTT Connectie fout: {e}")
...
def control_tv(tv_name, action, reason):
    ...
    def send_req():
        try:
            ...
            logger.error(f"❌ HTTP Fout op {tv_name}")
        ...
        if now.day != last_day:
            with data_lock:
                for name, state in tv_states.items():
                    state["remaining_minutes"] = state["config"].get("daily_limit", 120)
...
async def toggle_lock(tv_name: str):
    with data_lock:
        if tv_name in tv_states:
            ...
            control_tv(tv_name, action, "Handmatig")
