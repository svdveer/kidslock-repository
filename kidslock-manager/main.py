...
def send_action(ip, action):
    """Veilig acties sturen naar de TV met korte timeout om hangen te voorkomen"""
    try:
        requests.post(f"http://{ip}:8080/{action}", timeout=1.5)
        return True
    except:
        logger.warning(f"TV op {ip} reageerde niet op {action}")
        return False
...
try:
    ...
    logger.error("MQTT verbinding mislukt")
...
async def toggle(name: str):
    with data_lock:
        if name in tv_states:
            ...
            # Voer actie uit en update state
    ...
    # FIX: Lege string dwingt de browser terug naar de root van de add-on omgeving
